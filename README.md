...
```
cicero parse --template ./pop/ --dsl ./pop/sample.txt
...
data.json:
{
   "$class": "pop.zorro.templates.MyRequest",
   "input": "World"
}
```

```
cicero execute --template ./pop/ --dsl ./pop/sample.txt --data ./pop/data.json 
```
...
```
{
   ...
   "request":{
      "$class":"pop.zorro.templates.Request",
      "input":"World"
   },
   "response":{
      "$class":"pop.zorro.templates.Response",
